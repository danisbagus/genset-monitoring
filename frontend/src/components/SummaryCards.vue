    ...
    <BaseCard v-for="stat in stats" :key="stat.title" class="!p-5 hover:-translate-y-1 transition-all duration-300 group">
      <div class="flex items-center justify-between mb-4">
        <div :class="['p-2.5 rounded-xl transition-colors duration-300', stat.bg, stat.text]">
          ...
          <svg v-else-if="stat.icon === 'warning'" xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><line x1="12" x2="12" y1="9" y2="13"/><line x1="12" x2="12.01" y1="17" y2="17"/></svg>
          <svg v-else-if="stat.icon === 'critical'" xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" x2="12" y1="8" y2="12"/><line x1="12" x2="12.01" y1="16" y2="16"/></svg>
        </div>
        <div :class="['w-2 h-2 rounded-full', stat.value > 0 ? 'animate-pulse ' + stat.bg.replace('/10', '') : 'bg-slate-200 dark:bg-slate-800']"></div>
      </div>
      <div>
        <p class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.15em] mb-1">{{ stat.title }}</p>
        <h4 class="text-3xl font-black text-slate-900 dark:text-white transition-all group-hover:scale-110 origin-left duration-500">{{ stat.value }}</h4>
      </div>
    </BaseCard>
  </div>
</template>
